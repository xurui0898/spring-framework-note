...
public class DispatcherServlet extends FrameworkServlet {
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//处理器执行链对象
		// HandlerExecutionChain对象中两个变量
		//		List<HandlerInterceptor> interceptorList:拦截器集合
		//		Object handler:匹配到的handle对象（controller中的方法）
		...
		try {
			...
			try {
				//todo 1：解析multipart类型的请求，上传文件用的就是multipart类型的请求方式
				processedRequest = checkMultipart(request);
				//用来标记是否是multipart类型的请求
				multipartRequestParsed = (processedRequest != request);

				// Determine handler for the current request.
				//todo 2：根据请求获取HandlerExecutionChain对象
				//根据请求路径匹配，获取到 HandlerExecutionChain 对象
				...
				//todo 3：根据处理器获取HandlerAdapter
				//根据HandlerExecutionChain对象中的handler对象，获取到对应的HandlerAdapter对象
				...
				//todo 4：调用拦截器的preHandle方法，若返回false，处理结束
				if (!mappedHandler.applyPreHandle(processedRequest, response)) {
					return;
				}

				// Actually invoke the handler.
				//todo 5：调用handler实际处理请求，获取ModelAndView对象，这里会调用HandlerAdapter#handle方法处理请求，其内部会调用handler来处理具体的请求
				//通过HandlerAdapter来执行对应的controller方法
				// HandlerExecutionChain中的拦截器方法（PreHandle，PostHandle）在执行controller方法前后执行
			...
			//todo 7：处理分发结果，渲染视图(包含了正常处理和异常情况的处理)，将结果输出到客户端
			processDispatchResult(processedRequest, response, mappedHandler, mv, dispatchException);
		}
		catch (Exception ex) {
			//todo 8：调用拦截器的afterCompletion方法
			triggerAfterCompletion(processedRequest, response, mappedHandler, ex);
		}
		catch (Throwable err) {
			//todo 9：调用拦截器的afterCompletion方法
