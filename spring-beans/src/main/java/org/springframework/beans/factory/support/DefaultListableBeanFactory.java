...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	public void preInstantiateSingletons() throws BeansException {
		...
		// 根据beanName创建bean，
		// 		对于存在循环依赖的A，B类来说,先实例化A，后将A的实例化（空对象）放到三级缓存,在属性赋值的时候发现容器中没B，则去创建B对象，
		//		然后创建B对象，会先将B的实例化（空对象）放到三级缓存,发现容器中没A对象（容器中有就是相当于一级缓存中有），然后去找三级缓存中有没有，发现三级缓存有A的实例化对象，然后创建出B对象
		//		B对象创建成功后回过头去创建出A对象来
		for (String beanName : beanNames) {
			RootBeanDefinition bd = getMergedLocalBeanDefinition(beanName);
			//todo
			//    这个地方就是只会去创建 非抽象，单例，非懒加载的bean
			//    所以多例bean在定义的时候并不会去创建，只会在调用的时候创建。
			//    （所以一开始一级缓存中是没有多例bean对象的）
			// 注意点：
			// 	   一级缓存singletonObjects没有对应的多例bean，
			// 	   但是在parse阶段是已经扫描了多例bean，这个时候beanName集合中是有对应bean的
			if (!bd.isAbstract() && bd.isSingleton() && !bd.isLazyInit()) {
				if (isFactoryBean(beanName)) {
					// 实现FactoryBean的对象A,而 getObject()方法返回的是B对象
					// 这种情况下，容器中一级缓存 singletonObjects 存的就是 A对象
					// 然后通过容器对象 beanFactory.getBean() 拿到的就是 B对象
					//todo	通过 beanFactory.getBean(“A“)可以拿到B对象，
					// 		通过 beanFactory.getBean(“&A”)可以拿到A对象
					//
					// factoryBean在这个地方getBean() 走生命周期
	...
	public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
		...
		//会根据beanName获取beanDefinition
		//	这里也就是为什么 @Component和@bean可以注册相同beanName的bean不报错的原因（有个判断是否存在的逻辑），
		//总结：
		// 		两个 @Component注册相同beanName是会报错的
		//      但是一个@component，一个@bean注册相同的beanName是不会报错的
		//      经过测试：不同包下定义两个@bean注册两个相同的beanName也是不会报错的
		//所以：程序中定义了两个相同的beanName启动要分情况:
		//     1.如果是@Component层面定义了两个及以上的会报错
		//	   2.如果是定义了一个@component，若干个@bean，则不会报错
		...
		if (existingDefinition != null) {
			...
			if (hasBeanCreationStarted()) {
				...
				/**
				 * 将beanDefinition存放到beanDefinitionMap中
				 */
				this.beanDefinitionMap.put(beanName, beanDefinition);
