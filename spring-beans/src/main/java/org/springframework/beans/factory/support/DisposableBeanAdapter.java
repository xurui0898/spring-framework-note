...
class DisposableBeanAdapter implements DisposableBean, Runnable, Serializable {
	...
	public static boolean hasDestroyMethod(Object bean, RootBeanDefinition beanDefinition) {
		// 当前bean实现了 DisposableBean 接口（bean销毁接口）
	...
	private static String inferDestroyMethodIfNecessary(Object bean, RootBeanDefinition beanDefinition) {
		...
		if (destroyMethodName == null) {
			...
			// 如果 beanDefinition.resolvedDestroyMethodName == (inferred)，则进行推断
			if (AbstractBeanDefinition.INFER_METHOD.equals(destroyMethodName) ||
					(destroyMethodName == null && bean instanceof AutoCloseable)) {
				// Only perform destroy method inference or Closeable detection
				// in case of the bean not explicitly implementing DisposableBean
				destroyMethodName = null;
				//如果当前bean没有实现 DisposableBean
				if (!(bean instanceof DisposableBean)) {
					try {
						//判断当前bean是否含有 close 方法
						destroyMethodName = bean.getClass().getMethod(CLOSE_METHOD_NAME).getName();
					}
					catch (NoSuchMethodException ex) {
						try {
							//判断当前bean是否含有 shutdown 方法
		...
		// 如果 销毁方法存在返回true（进行bean的销毁），不存在返回false
