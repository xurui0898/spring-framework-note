...
public abstract class AbstractAdvisorAutoProxyCreator extends AbstractAutoProxyCreator {
	...
	protected Object[] getAdvicesAndAdvisorsForBean(
		...
		//上面这个方法作用：
		// 		匹配bean中的方法和之前的缓存切面集合进行匹配
		//		如果没有匹配到切面，则表示这个bean不需要代理，返回null，
		//		外面specificInterceptors对象 == null（DO_NOT_PROXY）
		//		则不会走if去创建代理对象，而是直接返回正常对象出去
		if (advisors.isEmpty()) {
			return DO_NOT_PROXY;
		}
	...
	protected List<Advisor> findEligibleAdvisors(Class<?> beanClass, String beanName) {
		//这里会找到项目中所有定义的切面 （拿到项目中所有的切面，返回一个切面列表）
		List<Advisor> candidateAdvisors = findCandidateAdvisors();
		//这里会会根据 execution 表达式去匹配对应的切面
		// 根据上面所有的切面集合 ，并根据入参的beanName确定bean，筛选对应的切面集合
		//
		//todo
		// 根据缓存中的所有切面 ，当前bean中的所有方法
		// 二者进行匹配
		// 最后返回符合当前bean的切面集合
